...
fs.readFile('html/data/menu.json',function(err,data){
    if(err)
        throw new Error('文件读取错误');
    navData = data;
    //请求轮播图数据
    fs.readFile('html/data/index/banner.json',function(err,data){
        if(err)
            throw new Error('文件读取错误');
        bannerData = data;
        //请求轮播图菜单的数据
        fs.readFile('html/data/index/menu.json',function(err,data){
            if(err)
                throw new Error('文件读取错误');
            menuData = data;
            //请求机酒自由行的数据
            fs.readFile('html/data/index/freeWalk.json',function(err,data){
                if(err){
                    throw new Error('文件读取错误');
                }
                fwData = data;
                //请求citywalk的数据
                fs.readFile('html/data/citywalk/cityWalkList.json',(err,data) => {
                    if(err)
                        throw new Error('文件读取错误');
                    cityData = data;
                    //启动服务器
