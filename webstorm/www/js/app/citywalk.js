...
define(['jquery'],function($){
    function getCityData(){
        $.ajax({
            url:'http://localhost:8000/citywalk',
            type:'get',
            success:handleData
        });
    }
    function handleData(data){
        ...
        $('.pageNum a').on('click',function(){
            ...
            showPageContent(startIndex,index,pageSize);
            showPage();
        ...
        $('.prePage').on('click',function () {
            //获取将要加载的页
            var index = parseInt($('.pageNum a.active').prev().html());
            //讲当前页的class = active移除，讲下一页的添加class
            $('.pageNum a').eq(index).removeClass('active').prev().addClass('active');
    ...
    //显示当前页的内容
    function showPageContent(startIndex,index,pageSize){
        if(startIndex == 0 || isNaN(index)){
